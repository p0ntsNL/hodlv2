...
class Bot:
    ...
    def calculate_close_price(self, open_price):
        ...
        if self.open_side == 'buy':
            return float(open_price) + ((float(open_price) / 100) * float(self.config.PERC_CLOSE))
        else:
            return float(open_price) - ((float(open_price) / 100) * float(self.config.PERC_CLOSE))
    ...
    def calculate_next_open_price(self, open_price):
        ...
        if self.open_side == 'buy':
            return float(open_price) - ((float(open_price) / 100) * float(self.config.PERC_OPEN))
        else:
            return float(open_price) + ((float(open_price) / 100) * float(self.config.PERC_OPEN))
            ...
            if self.open_orders[0] and self.closed_orders[0] and self.ok:
                ...
                for m in self.config.MARKETS:
                    ...
                    if self.ticker_data[0] and self.ok:
                        ...
                        if self.check_next_price(float(self.next_price), float(self.ticker_data['last'])) and self.ok:
                            self.new = True
                        ...
                        if self.new and self.ok:
                            ...
                            if self.balances[0] and self.ok:
                                ...
                                if self.balance_check(self.quote_balance, self.config.TRADE_VALUE):

                                    self.trade_value = self.calculate_trade_value(self.ticker_data['last'])
                                    self.market_order = self.ccxt.create_market_order(self.market, self.open_side, self.trade_value)
                                    if self.market_order[0] and self.ok:

                                        self.open_order_id = self.market_order[1]['info']['txid'][0]
                                        self.open_order_details = self.ccxt.fetch_order(self.market, self.open_order_id)

                                        if self.open_order_details[0] and self.ok:

                                            self.open_order_details = self.open_order_details[1]

                                            self.close_price = self.calculate_close_price(self.open_order_details['average'])
                                            self.next_price = self.calculate_next_open_price(self.open_order_details['average'])

                                            self.close_trade_value = self.profit_in_trade_value(self.config.PROFIT_IN, self.open_order_details['filled'], self.open_order_details['cost'], self.close_price)

                                            self.close_limit_order = self.ccxt.create_limit_order(self.market, self.close_side, self.close_trade_value, self.close_price)
                                            if self.close_limit_order[0] and self.ok:

                                                self.close_order_details = self.close_limit_order[1]

                                                self.add_trade_to_local_data(self.local_data, self.open_order_details, self.close_order_details)
                                                self.add_next_price_to_local_data(self.local_data, self.next_price)

                                                self.notify.send('<b>Trade opened</b><br>Market: {}<br>Side: {}<br>Price: {} {}<br>Amount: {} {}<br>Cost: {} {}<br><br><b>Closing order</b><br>Close price: {} {}<br>Close amount: {} {}<br>Close cost: {} {}'.format(self.market, self.open_side, self.open_order_details['average'], self.quote, self.open_order_details['filled'], self.base, self.open_order_details['cost'], self.quote, self.close_order_details['price'], self.quote, self.close_trade_value, self.base, (float(self.close_order_details['price']) * float(self.close_trade_value)), self.quote), 'INFO')

                                            else:
                                                self.notify.end('{}: Unable to create limit order...'.format(self.market), 'ERROR')

                                        else:
                                            self.notify.send('{}: Unable to fetch market order data...'.format(self.market), 'ERROR')

                                    else:
                                        self.notify.send('{}: Unable to create market order...'.format(self.market), 'ERROR')
                        ...
                        for o in self.closed_orders:
                            ...
                            if o['id'] in self.local_data['active_trades'][self.market]:
                                ...
                                self.local_data['active_trades'][self.market][o['id']]['close'] = o
                                self.save_local_data(self.local_data)

                                if o['status'] == 'canceled' or o['status'] == 'expired' or o['status'] == 'rejected':

                                    del self.local_data['active_trades'][self.market][o['id']]
                                    self.save_local_data(self.local_data)

                                    self.notify.send('<b>Trade closed ({})</b><br>Market: {}'.format(o['status'], self.market), 'WARNING')

                                elif o['status'] == 'closed':

                                    self.closed_open_order = self.local_data['active_trades'][self.market][o['id']]['open']
                                    self.closed_close_order = self.local_data['active_trades'][self.market][o['id']]['close']
                                    self.profit_in = self.local_data['active_trades'][self.market][o['id']]['profit_in']

                                    self.profit = self.calculate_profit(self.profit_in, self.closed_open_order, self.closed_close_order)

                                    self.local_data['finished_trades'][self.market][o['id']] = self.local_data['active_trades'][self.market][o['id']]
                                    self.local_data['profits'][self.market][o['id']] = { "profit":self.profit[0], "profit_currency":self.profit[1] }
                                    del self.local_data['active_trades'][self.market][o['id']]
                                    self.save_local_data(self.local_data)

                                    # Calculate total profit of the profit currency
                                    self.total_profit = self.calculate_total_profit()
                                    self.total_profit_pretty = self.return_total_profit(self.total_profit)

                                    self.notify.send('<b>Trade closed</b><br>Market: {}<br>Profit:{:.8f} {}<br><br>{}'.format(self.market, self.profit[0], self.profit[1], self.total_profit_pretty), 'INFO')


                    else:
                        self.notify.send('{}: Unable to retrieve ticker data from exchange...'.format(self.market), 'ERROR')

            else:
                self.notify.send('{}: Unable to retrieve open and closed order data from exchange...'.format(self.market), 'ERROR')
